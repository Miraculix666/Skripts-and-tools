...
#
# Autor:      PS-Coding
# Version:    17.1 - Korrektur der Ziel-Verzeichnisstruktur (drivers/drivers)
# Datum:      2025-09-09
#
# ANWENDUNG:
...
show_help() {
    echo "OPSI All-in-One Treiber-Skript V17.1"
    echo ""
...
# KORREKTUR: Pfad an OPSI-Standard-Struktur (drivers/drivers) angepasst.
...
# --- Skript-Start ---
log_message "INFO" "Starte OPSI Treiber-Sortierungsskript V17.1"
...
if [[ $REPLY =~ ^[Jj]$ ]]; then
    ...
    log_message "INFO" "Sammle Hardware-Daten von allen OPSI-Clients (dies kann dauern)..."
    client_ids=$($OPSI_ADMIN_CMD method getClientIds_list)
    ...
    for client_id in $client_ids; do
        ...
        hw_data=$($OPSI_ADMIN_CMD method getHostHardware_hash "$client_id" || true)
        echo "$hw_data" | grep -E -o "'(pci|usb)\[[^]]*\]': u'[^']+'" | sed -e "s/'pci\[[^]]*\]': u'//" -e "s/'usb\[[^]]*\]': u'//" -e "s/'$//" | while read -r id; do
            if [[ $id =~ (PCI|USB)DEV_([0-9A-Fa-f]+)&VEN_([0-9A-Fa-f]+) ]]; then
                bus_type=${BASH_REMATCH[1]}; device_id=${BASH_REMATCH[2]}; vendor_id=${BASH_REMATCH[3]}
                if [ "$bus_type" == "PCI" ]; then
                    normalized_id="PCI\\VEN_${vendor_id^^}&DEV_${device_id^^}"
                else
                    normalized_id="USB\\VID_${vendor_id^^}&PID_${device_id^^}"
                fi
                NEEDED_HW_IDS["$normalized_id"]=1
            fi
        done
    done
    log_message "OK" "Audit-Analyse abgeschlossen. ${#NEEDED_HW_IDS[@]} eindeutige, benötigte Hardware-IDs gefunden."
else
    log_message "INFO" "Standard-Integration: Alle gefundenen Treiber werden verarbeitet."
fi

# PHASE 2: CACHE-ABGLEICH
print_header "Phase 2/5: Gleiche Cache-DB mit dem Dateisystem ab..."
TEMP_DB=$(mktemp)
...
log_message "OK" "Cache-DB-Abgleich abgeschlossen. ${pruned_count} von ${entry_count} Einträgen entfernt."
...
find "${UNSORTED_DRIVER_DIR}" -type f -iname "*.inf" -print0 2>/dev/null | while IFS= read -r -d '' inf_file; do
    ...
    driver_ver_line=$(iconv -f UTF-16LE -t UTF-8 "${inf_file}" 2>/dev/null | grep -i -m 1 "DriverVer" || true)
    ...
    ids=$(iconv -f UTF-16LE -t UTF-8 "${inf_file}" 2>/dev/null | grep -E -o -i 'PCI\\VEN_[0-9A-F]{4}&DEV_[0-9A-F]{4}|USB\\VID_[0-9A-F]{4}&PID_[0-9A-F]{4}' || true | tr -d '\r' | sort -u)
    if [ -z "$ids" ]; then
        log_message "WARN" "Keine unterstützten Hardware-IDs in ${inf_file} gefunden. Überspringe."
        continue
    fi
...
while IFS='|' read -r inf_path checksum version hwids || [[ -n "$inf_path" ]]; do
    ...
    for id_line in "${hwid_array[@]}"; do
        if [ "$INTEGRATE_ONLY_NEEDED_DRIVERS" = true ] && [ -z "${NEEDED_HW_IDS[$id_line]}" ]; then
            continue
        fi
        if [ -z "${LATEST_DRIVERS[$id_line]}" ]; then
            LATEST_DRIVERS[$id_line]="${version};${inf_path}"
        else
            existing_ver_norm=$(echo "${LATEST_DRIVERS[$id_line]}" | cut -d';' -f1)
...
for id_line in "${!LATEST_DRIVERS[@]}"; do
    ...
    if [[ $id_line =~ ^PCI\\VEN_([0-9A-F]{4})\&DEV_([0-9A-F]{4})$ ]]; then
    ...
    local integration_mode_subdir="manual"
    if [ "$INTEGRATE_ONLY_NEEDED_DRIVERS" = true ]; then
        integration_mode_subdir="byAudit"
    fi

    SORT_TYPE="additional" # Standard
    if [[ "${inf_file}" == *"/preferred/"* ]]; then SORT_TYPE="preferred"; fi
    if [[ "${inf_file}" == *"/excluded/"* ]]; then SORT_TYPE="excluded"; fi

    # Erstellt die finale Ziel-Pfadstruktur, z.B. .../drivers/drivers/additional/byAudit/pciids/8086/1234
    final_target_path="${TARGET_DRIVER_DIR}/${SORT_TYPE}/${integration_mode_subdir}/${TYPE}/${VENDOR^^}/${DEVICE^^}"
