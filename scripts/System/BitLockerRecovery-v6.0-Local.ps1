### Das finale Skript (Version 6.6 - Stabile Einzel-Installation)

Dies ist die finale Version, die alle unsere Erkenntnisse vereint und den nachweislich funktionierenden Installations-Weg nutzt.

**Dateiname:** `Start-IntelligentBitLockerRecovery-v6.6-FINAL.ps1`

```powershell
# Start-IntelligentBitLockerRecovery-v6.6-FINAL.ps1 - Finale Version mit stabiler Einzel-Installation der Voraussetzungen.
#Requires -RunAsAdministrator

<#
.SYNOPSIS
  Eine finale, hochrobuste Lösung zur Rettung von Daten aus beschädigten BitLocker-Laufwerken auf ein lokales Laufwerk.
.DESCRIPTION

.NOTES
  Autor: PS-Coding (AI-Assisted)
  Version: 6.6 (Local Recovery - FINAL)
  Änderungen:
  - v6.6: FINAL - Installationslogik korrigiert. Fehlende Komponenten werden nun einzeln installiert, um einen Fehler im 'Add-WindowsCapability'-Cmdlet zu umgehen.
  - v6.5: Optimierte Voraussetzungs-Installation (fehlerhaft).
#>
...
begin {
    ...
    # --- START: Finale Funktion zur Überprüfung und Installation von Abhängigkeiten ---
    function Ensure-Prerequisites {
        Write-Verbose "Prüfe erforderliche Komponenten..."
        $restartRequired = $false

        $modulesToVerify = @(
            @{ CommandName = "New-VHD"; CapabilityName = "Rsat.Hyper-V.Tools~~~~0.0.1.0"; FriendlyName = "Hyper-V Tools" }
            @{ CommandName = "repair-bde"; CapabilityName = "Rsat.BitLocker.Recovery.Tools~~~~0.0.1.0"; FriendlyName = "BitLocker Recovery Tools" }
        )

        foreach ($module in $modulesToVerify) {
            if (-not (Get-Command -Name $module.CommandName -ErrorAction SilentlyContinue)) {
                Write-Warning "Komponente '$($module.FriendlyName)' nicht gefunden. Versuche automatische Installation..."
                if ($PSCmdlet.ShouldProcess("Windows-Fähigkeit '$($module.FriendlyName)'", "Installieren")) {
                    try {
                        Add-WindowsCapability -Online -Name $module.CapabilityName -ErrorAction Stop
                        Write-Host "$($module.FriendlyName) erfolgreich installiert." -ForegroundColor Green
                        $restartRequired = $true
                    }
                    catch {
                        throw "Fehler bei der Installation von '$($module.FriendlyName)'. Bitte versuchen Sie die manuelle Installation des Pakets '$($module.CapabilityName)'."
                    }
                }
                else {
                    throw "Installation von '$($module.FriendlyName)' vom Benutzer abgelehnt. Skript wird beendet."
                }
            }
            else {
                Write-Verbose "Komponente '$($module.FriendlyName)' ist bereits vorhanden."
            }
        }

        if ($restartRequired) {
            Write-Host "----------------------------------------------------------------" -ForegroundColor Yellow
            Write-Host "WICHTIG: Es wurden neue Windows-Komponenten installiert." -ForegroundColor Yellow
            Write-Host "Bitte schließen Sie dieses PowerShell-Fenster und starten Sie es erneut" -ForegroundColor Yellow
            Write-Host "mit der Option 'Als Administrator ausführen', bevor Sie das Skript erneut starten." -ForegroundColor Yellow
            Write-Host "----------------------------------------------------------------" -ForegroundColor Yellow
            exit
        }
        else {
            Write-Host "Alle Voraussetzungen sind erfüllt." -ForegroundColor Green
            Write-Host "----------------------------------------------------------------"
        }
    }
...
process {
    ...
    try {
        Clear-Host
        Write-Host "================================================================" -ForegroundColor Yellow
        Write-Host "      Intelligentes BitLocker Recovery Skript v6.6 (Final)" -ForegroundColor Yellow
        Write-Host "================================================================" -ForegroundColor Yellow
