...
$LogFile = Join-Path <span class="math-inline">PSScriptRoot "ADUserCreation\_Log\_</span>(Get-Date -Format 'yyyyMMdd_HHmmss').log"
...
function Test-PasswordComplexity {
    ...
    foreach ($rule in $rules) {
        if (-not (& $rule)) { 
            Write-Verbose "Passwortregel nicht erfüllt: $(<span class="math-inline">rule \-replace '^\.\*\\\{\|\\\}\.\*</span>')"
            return $false 
        }
    }
...
function New-ADUserFromTemplate {
    ...
    try {
        ...
        $userParams = @{
            ...
            Path                  = $ouPath
            UserPrincipalName     = "<span class="math-inline">NewSam@</span>((Get-ADDomain).DNSRoot)"
        }
        ...
        foreach ($key in $OptionalAttributes.Keys) {
            ...
            if (-not $userParams.ContainsKey($key)) {
                ...
                Write-Verbose "Übernehme optionales Attribut '<span class="math-inline">key' mit Wert '</span>($OptionalAttributes[$key])'."
            } else {
